...
#if defined(NDEBUG)
# error "VuiCash cannot be compiled without assertions."
#endif
...
std::unique_ptr<VuiCashState> globalState;
...
bool fAddressIndex = false; // vuicash
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if(tx.HasCreateOrCall()){
        ...
        uint64_t minGasPrice = vuicashDGP.getMinGasPrice(::ChainActive().Tip()->nHeight + 1);
        ...
        VuiCashTxConverter converter(tx, NULL, NULL, contractflags);
        ExtractVuiCashTX resultConverter;
        if(!converter.extractionVuiCashTransactions(resultConverter)){
        ...
        std::vector<EthTransactionParams> vuicashETP = resultConverter.second;

        dev::u256 sumGas = dev::u256(0);
        dev::u256 gasAllTxs = dev::u256(0);
        for(VuiCashTransaction vuicashTransaction : vuicashTransactions){
            sumGas += vuicashTransaction.gas() * vuicashTransaction.gasPrice();
            ...
            if(txMinGasPrice != 0) {
                txMinGasPrice = std::min(txMinGasPrice, vuicashTransaction.gasPrice());
            } else {
                txMinGasPrice = vuicashTransaction.gasPrice();
            }
            VersionVM v = vuicashTransaction.getVersion();
            if(v.format!=0)
            ...
            if(vuicashTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
            ...
            if(vuicashTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
            ...
            if(vuicashTransaction.gas() > UINT32_MAX)
            ...
            gasAllTxs += vuicashTransaction.gas();
            ...
            if(v.rootVM!=0 && (uint64_t)vuicashTransaction.gasPrice() < minGasPrice)
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // vuicash
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // vuicash
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit, CAmount nAmount){
    ...
    VuiCashTransaction callTransaction;
    if(addrContract == dev::Address())
    {
        callTransaction = VuiCashTransaction(nAmount, 1, dev::u256(gasLimit), opcode, nonce);
    }
    else
    {
        callTransaction = VuiCashTransaction(nAmount, 1, dev::u256(gasLimit), addrContract, opcode, nonce);
    }
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<VuiCashTransaction>(1, callTransaction), blockGasLimit, pblockindex);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path vuicashDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(vuicashDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(vuicashDir.string(), std::ios::in | std::ios::out);
...
bool ByteCodeExec::performByteCode(dev::eth::Permanence type){
    for(VuiCashTransaction& tx : txs){
        ...
        if(!tx.isCreation() && !globalState->addressInUse(tx.receiveAddress())){
            ...
            result.push_back(ResultExecute{execRes, VuiCashTransactionReceipt(dev::h256(), dev::h256(), dev::u256(), dev::eth::LogEntries()), CTransaction()});
            continue;
        }
...
bool VuiCashTxConverter::extractionVuiCashTransactions(ExtractVuiCashTX& vuicashtx){
    ...
    std::vector<VuiCashTransaction> resultTX;
    ...
    vuicashtx = std::make_pair(resultTX, resultETP);
    return true;
}

bool VuiCashTxConverter::receiveStack(const CScript& scriptPubKey){
    sender = false;
...
bool VuiCashTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
VuiCashTransaction VuiCashTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    VuiCashTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = VuiCashTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = VuiCashTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
...
size_t VuiCashTxConverter::correctedStackSize(size_t size){
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // vuicash
    VuiCashDGP vuicashDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setVuiSchedule(vuicashDGP.getGasSchedule(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // vuicash
...
///////////////////////////////////////////////////////////////////////////////////////// vuicash
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            VuiCashTxConverter convert(tx, &view, &block.vtx, contractflags);

            ExtractVuiCashTX resultConvertVuiCashTX;
            if(!convert.extractionVuiCashTransactions(resultConvertVuiCashTX)){
            ...
            if(!CheckMinGasPrice(resultConvertVuiCashTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertVuiCashTX.first, blockGasLimit, pindex->pprev);
            ...
            for(VuiCashTransaction& qtx : resultConvertVuiCashTX.first){
            ...
            if (fLogEvents && !fJustCheck)
            {
                ...
                for(size_t k = 0; k < resultConvertVuiCashTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{
                        ...
                        tx.GetHash(),
                        uint32_t(i),
                        resultConvertVuiCashTX.first[k].from(),
                        resultConvertVuiCashTX.first[k].to(),
                        ...
                        resultConvertVuiCashTX.first[k].getNVout(),
...
bool CChainState::ConnectTip(BlockValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // vuicash
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // vuicash
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // vuicash
            globalState->setRootUTXO(oldHashUTXORoot); // vuicash
...
bool SignBlockHWI(std::shared_ptr<CBlock> pblock, CWallet& wallet, std::vector<unsigned char>& vchSig)
{
    ...
    VuiCashLedger &device = VuiCashLedger::instance();
...
bool SignBlockLedger(std::shared_ptr<CBlock> pblock, CWallet& wallet, std::vector<unsigned char>& vchSig)
{
    ...
    {
        std::string errorMessage = VuiCashLedger::instance().errorMessage();
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (pindex = ::ChainActive().Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        ///////////////////////////////////////////////////////////////////// // vuicash
        uint32_t sizeBlockDGP = vuicashDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for VuiCash PoS we need to sync this after every block to ensure txdb is populated for
...
CAmount GetTxGasFee(const CMutableTransaction& _tx)
{
    ...
    if(tx.HasCreateOrCall())
    {
        ...
        VuiCashTxConverter convert(tx, NULL, NULL, contractflags);
